...
//send chunked message stored in cache to client
...
//receive and send chunked and large message
int handleLargeMsg(std::vector<char> &buffer, int recvLen, int client_fd, int server_fd, Message &response){
  ...
  //handle response with "Content-Length"
  ...
  //handle response with "Transfer-Encoding: chunked"
...
//recv and send message within a tunnel for CONNECT
...
void runProxy(int client_fd, LRUCache *cache) {
  ...
  //receive the first part of request
  ...
  //create a socket to connect target server
  ConnectSocket cs;
  try{
  ...
  /*******************************************************
    handle GET request
    Cached responses to GET requests, 
    following rules of expiration time and re-validation
  ********************************************************/
  if (method == "GET") {
  ...
  ********************************************************/
  ...
  /*******************************************************
    ...
    build a tunnel for client and server
    exit when recv returns 0
  ********************************************************/
    ...
    while (1) {
      ...
      //wait for client or server to send message
...
int main(int argc, char **argv) {
  ...
  //crete a socket to accept clients' requests
  ListenSocket ls; 
  try {
  ...
  //create a LRU cache of size 100
  LRUCache cache(100);
  while (1) {
    int client_fd;
    try{
